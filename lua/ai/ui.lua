	...
	if key ~= nil then
		...
		for _, k in ipairs(keys) do
			vim.api.nvim_buf_set_keymap(
				...
				"n",
				k,
				"<cmd>lua vim.api.nvim_set_current_win(require('ai.ui').output_popup.winid)<cr>",
				opts
			)
		end
	end
	...
	if key ~= nil then
		...
		for _, k in ipairs(keys) do
			vim.api.nvim_buf_set_keymap(
				...
				"n",
				k,
				"<cmd>lua vim.api.nvim_set_current_win(require('ai.ui').input_popup.winid)<cr>",
				opts
			)
		end
	end
...
M.set_destroy_key_mappings = function(input_buffer)
	...
	for _, key in ipairs(mappings) do
		vim.api.nvim_buf_set_keymap(
			...
			"n",
			key,
			"<cmd>lua require('ai.ui').destroy_ui()<cr>",
